...
| <a name="input_target_group_health_check_port"></a> [target\_group\_health\_check\_port](#input\_target\_group\_health\_check\_port) | (Optional) The non-standard port to ping health checks against.  If omitted, the traffic port is used. | `number` | `-1` | no |
